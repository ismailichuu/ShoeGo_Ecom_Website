...
function getFinalPriceWithLabel(product) {
  ...
  const { price: prodPrice, discount: prodDisc } = calculateDiscountPrice(basePrice, product.discount);
  const { price: catPrice, discount: catDisc } = calculateDiscountPrice(basePrice, product.categoryId?.[0]?.discount || '');
