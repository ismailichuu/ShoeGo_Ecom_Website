...
export const getAddresses = async (req, res) => {
  try {
    ...
    logger.error('getAddresses:', error.toString());
  }
};
...
export const getEditAddress = async (req, res) => {
  try {
    ...
    logger.error('getEditAddress:', error.toString());
  }
};
...
export const getSelectAddress = async (req, res) => {
  try {
    ...
    const activeItems = cart.cartItems.filter(
      (item) => item.productId && item.productId.isActive
    );

    if (activeItems.length !== cart.cartItems.length) {
      await Cart.updateOne({ userId }, { $set: { cartItems: activeItems } });
    ...
    logger.error('getSelectAdderess:', error);
    return res.redirect('/cart');
  }
};
...
export const getAddNewAddress = (req, res) => {
  try {
    ...
    logger.error('getAddNewAddress:', error.toString());
  }
};
...
export const getEditAddressCheckout = async (req, res) => {
  try {
    ...
    const address = await Address.findById(addressId);
    const msg = req.session.err || null;
    res.render('user/editAddressPage', {
      layout: 'checkOutLayout',
      msg,
      cartId: cart._id,
      address,
    ...
    logger.error('getEditAddressCheckout:', error.toString());
  }
};
...
export const handleEditAddressCheckout = async (req, res) => {
  ...
  try {
    ...
    (address.mobileNumber = phone), (address.pincode = pincode);
    address.locality = locality;
    address.houseNo = houseNo;
    address.city = city;
    address.state = state;
