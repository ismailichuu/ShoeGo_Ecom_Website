...
export const handleIncreaseCount = async (req, res) => {
  try {
    ...
    const totalProductQuantityInCart = cart.cartItems.reduce(
      (total, cartItem) => {
    ...
    if (item.quantity >= 5) {
      return res
        .status(400)
        .json({ success: false, message: 'Maximum quantity reached (5)' });
    }
    ...
    logger.error('Increase count error:', error);
    return res
      .status(500)
...
export const getOrderSummary = async (req, res) => {
  try {
    ...
    logger.error('Order Summary Error:', err);
...
export const handleDecreaseCount = async (req, res) => {
  try {
    ...
    const updatedQuantity =
      itemIndex === -1 ? 0 : cart.cartItems[itemIndex]?.quantity || 0;
...
export const deleteCartItem = async (req, res) => {
  ...
  try {
    ...
    logger.error('deleteCartItems:', err);
