...
const createChart = (ctx, labels, data, color, type = 'bar') => {
    ...
    const backgroundColors = Array.isArray(color) ? color : Array(data.length).fill(color);
...
const updateDashboard = async (filter) => {
    try {
        ...
        productChart = createChart(productsCtx, productLabels, productSales, dynamicColors, 'bar');
        ...
        categoryChart = createChart(categoriesCtx, categoryLabels, categorySales, dynamicColorsf, 'doughnut');
        ...
        brandChart = createChart(brandsCtx, brandLabels, brandSales, 'rgba(16,185,129,0.7)');
